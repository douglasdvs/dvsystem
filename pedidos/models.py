...
class Pedido(models.Model):
    class Status(models.TextChoices):
        PENDENTE = "P", "Pendente"
        APROVADO = "A", "Aprovado"
        EMBALADO = "B", "Embalado para Envio"
        ENVIADO = "E", "Enviado"
        ENTREGUE = "T", "Entregue"
    ...
    # Pessoal
    vendedor = models.ForeignKey(Vendedor, on_delete=models.SET_NULL, null=True, blank=True, related_name="pedidos")
    usuario = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name="pedidos_criados")
    criado_por = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="pedidos_criados_por"
    )
    atualizado_por = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, related_name="pedidos_atualizados_por"
    )
    ...
    arquivo_arte = models.FileField(
        upload_to="pedidos/artes/", null=True, blank=True, help_text="JPG, PNG ou PDF até 2MB"
    )
    ...
    valor_recebido = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, help_text="Valor recebido do cliente"
    )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["arte_aprovada"]),
        ]
    ...
    def entrega_em_atraso(self):
        if not self.data_entrega_prevista or self.data_entrega_realizada:
            return False
        return self.status not in ["T", "C"] and self.data_entrega_prevista < timezone.now().date()
    ...
    def aplicar_cupom(self, codigo_cupom):
        ...
        cupom = CupomDesconto.objects.get(codigo=cupom)
...
class CupomDesconto(models.Model):
    ...
    def esta_valido(self):
        agora = timezone.now()
        return (
            self.ativo and self.data_inicio <= agora <= self.data_fim and self.quantidade_usada < self.quantidade_maxima
        )
