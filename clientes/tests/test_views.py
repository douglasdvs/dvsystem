...
class TestTipoClienteViews:
    ...
    def test_criar_tipo(self, client):
        ...
        data = {"nome": "Novo Tipo", "descricao": "Descrição do novo tipo", "desconto_padrao": "15.00"}
        response = client.post(url, data)
        assert response.status_code == 302
    ...
    def test_editar_tipo(self, client, tipo_cliente):
        ...
        data = {"nome": "Tipo Editado", "descricao": "Nova descrição", "desconto_padrao": "20.00"}
        response = client.post(url, data)
        assert response.status_code == 302
...
class TestEnderecoViews:
    ...
    def test_editar_endereco(self, client, cliente, endereco):
        ...
        assert (
            endereco.logradouro == "Avenida Editada"
        ), "O logradouro do endereço deve ser atualizado para 'Avenida Editada'."
    ...
    def test_excluir_endereco(self, client, cliente, endereco):
        ...
        assert not cliente.enderecos.filter(
            id=endereco.id
        ).exists(), "O endereço não deve mais existir no banco de dados."
